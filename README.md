...
* Parse project links on the respective online registry: [https://pypi.org/project/bleach/](https://pypi.org/project/bleach/)
    * Homepage -> [https://github.com/mozilla/bleach](https://github.com/mozilla/bleach)
* Return link that points to a GitHub Repository


### Maven Example:
* Maven projects were non-trivial, and we had the least success with them. If others have a better solution, I'm open to it. 
...
* Pull the latest version of the package from the response
...
Our process only obtained ~56% of the missing source code links. The other advisories will need more thought (primarily for Maven packages). Feel free to add more suggestions. 

NPM Notes: 330 of the advisories had matches, but we excluded them because NPM set the source code repository to (https://github.com/npm/security-holder) due to the package being yanked for malicious activity.